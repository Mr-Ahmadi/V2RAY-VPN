...
CA_DIR = os.environ.get("BRIDGE_CA_DIR", "").strip() or os.path.join(_PROJECT_ROOT, "ca")
CA_KEY_FILE = os.environ.get("BRIDGE_CA_KEY_FILE", "").strip() or os.path.join(CA_DIR, "ca.key")
CA_CERT_FILE = os.environ.get("BRIDGE_CA_CERT_FILE", "").strip() or os.path.join(CA_DIR, "ca.crt")
...
class MITMCertManager:
    ...
    def _create_ca(self):
        ...
        os.makedirs(os.path.dirname(CA_KEY_FILE) or CA_DIR, exist_ok=True)
        os.makedirs(os.path.dirname(CA_CERT_FILE) or CA_DIR, exist_ok=True)
